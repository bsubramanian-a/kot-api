...
const communityTopicService = require("../services/communityTopic");

const action = {};
 
action.addCommunityTopic = async (data) => {
  logger.info("Adding new Community Topic", data);
  try {
    const communityData = await communityTopicService.addCommunityTopic(data);
...
action.updateCommunityTopic = async (data) => {
  logger.info("updating new Community Topic", data);
  try {
    let query = {
      _id: data._id
    }
    const communityTopicData = await communityTopicService.updateCommunityTopic(query, data);
    return defaultFunction.success({
      response: null,
      message: "Community Topic updated successfully!",
      total: 1
    });
  } catch (error) {
    logger.error("Failed while updating community topic", error);
    return defaultFunction.somethingWentWrong({ error: error, message: "Failed while updating community topic" });
  }
};
...
action.getAllCommunityTopicsByCategory = async (query) => {
  try {
    if (!query || !query.category) {
      throw "category can not be null";
    }
    ...
    const communityTopicDetail = await communityTopicService.getAllCommunityTopicsByCategory(queryData);
    
    return defaultFunction.success({
      response: communityTopicDetail,
      message: "Community Topic listed successfully",
      total: communityTopicDetail.length
    });

  } catch (error) {
    logger.error("Failed while listing community topic", error);
    return defaultFunction.somethingWentWrong({ error });
  }
};

action.getAllCommunityTopics = async (query) => {
    try {
      let queryData={}
  
      const communityTopicsDetail = await communityTopicService.getAllCommunitiesTopics(queryData);
