...
action.addReport = async (data) => {
  logger.info("Adding new report", data);
  try {
    const postData = await reportService.addReport(data);
    ...
    logger.error("Failed while adding report", error);
    return defaultFunction.somethingWentWrong({ error: error, message: "Failed while adding peport" });
  }
};
